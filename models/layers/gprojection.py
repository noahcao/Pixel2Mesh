...
class GProjection(nn.Module):
    ...
    def project(self, img_shape, img_feat, sample_points):
        ...
        if self.tensorflow_compatible:
            ...
            half_resolution = (feature_shape - 1) / 2
            points_w = (sample_points[:, :, 0] / (img_shape[0] / feature_shape[0]) -
                        half_resolution[0]) / half_resolution[0]
            points_h = (sample_points[:, :, 1] / (img_shape[1] / feature_shape[1]) -
                        half_resolution[1]) / half_resolution[1]
            points_w = torch.clamp(points_w, min=-1, max=1)
            points_h = torch.clamp(points_h, min=-1, max=1)
            sample_points = torch.stack([points_w, points_h], dim=-1)
